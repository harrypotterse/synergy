...
<?php
$class = new Achieve();
$query = "select * from  our_selection  where id = ? ";
$array_var = array();
                                                ...
                                                <div class='col-lg-offset-2 col-lg-10 '>
                                                    ...
                                                    <div class="input-group margin">
                                                        <select class="form-control input-lg" id="sel1" name="Icon">
                                                            <?php
                                                            $query = "select DISTINCT Icon  from  our_selection ";
                                                            $array = array();
                                                            $rows = $class->sql_feth($Data_communication, $query, $array);
                                                            // print_r($rows);
                                                            if (count($rows) > 0) :
                                                                foreach ($rows as $value):
                                                                    $Icon = $value['Icon'];
                                                                    $trimmed = str_replace('medium-icon', '', $Icon);
                                                                    $trimmed2 = strtoupper(str_replace('icon-', '', $trimmed));
                                                                    echo "<option value='$Icon'>$trimmed2</option>";
                                                                endforeach;
                                                            endif;
                                                            ?>
                                                        </select>
