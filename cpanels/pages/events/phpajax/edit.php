...
if (!file_exists($file_tmp)) {
    $msgerr .= "<div class='alert alert-danger alert-autocloseable-danger'>من فضلك قم بتحميل الايكون</div>";
    $true = FALSE;
}
...
if ($true == TRUE):
    try {
        $array_var[] = files_modified($file_tmp, "../../../../Public/icon/", $_POST['file'], $file_name);
        ...
        $SQL = "UPDATE `events` SET `Icon`= ?,`Title`= ?,`Explanation`= ?  WHERE `events`.`id`= ?;";
        $array = array();
